...
export const createPost = async (req, res) => {
    try {
        ...
        if (!text && !img)
            return res.status(400).json({ success: false, error: "Post must have text or image" });
...
export const getLikedPosts = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(userId);
        if (!user) {
            return res.status(404).json({ success: false, error: "Usern not found" });
        }
...
export const getfollowingPosts = async (req, res) => {
    try {
        ...
        const feedPosts = await Post.find({ user: { $in: following } }).sort({ createAt: -1 }).populate({
            path: "user",
            select: "-password"
        }).populate({
            path: "comments.user",
